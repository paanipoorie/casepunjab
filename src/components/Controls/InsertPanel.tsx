...
export const InsertPanel: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const metadata = {
      category: 'Spliced Stop',
      subtitle: `Inserted after ${validTarget}`,
      badge: 'DETOUR',
      icon: 'MapPin',
      description: `Inserted ${newName.trim()} into the tour after ${validTarget}.`
    };
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
      <div className="mb-4">
        <h4 className="text-sm font-bold text-slate-100 flex items-center gap-2">
          ...
          <span>Insert {scenario.nodeLabel} After an Existing Stop</span>
        </h4>
        <p className="text-xs text-slate-400 mt-0.5">
          Splice a new node between two existing landmarks by updating two pointers.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">
              Insert a new stop after:
            </label>
            <select
              value={validTarget}
              onChange={(e) => setTargetName(e.target.value)}
              className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-sm text-slate-100 focus:outline-none focus:ring-1 focus:ring-amber-500 font-sans"
            >
              {nodes.map((n, idx) => (
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-amber-500 font-sans"
            />
          </div>
        ...
        {validTarget && (
          <div className="p-3 rounded-lg bg-slate-950 border border-slate-800/80 font-mono text-xs">
            <div className="text-slate-400 text-[11px] mb-1">Pointer rewire preview:</div>
        ...
        <div className="flex justify-end pt-1">
          <button
            ...
            className="px-5 py-2 rounded-lg bg-amber-500 text-slate-950 font-bold text-sm hover:bg-amber-400 disabled:opacity-50 transition-all flex items-center gap-1.5 active:scale-98 shadow-sm"
          >
