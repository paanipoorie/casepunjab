...
import { 
  ...
  Layers, CheckCircle2, AlertTriangle, Info, Terminal, Zap
} from 'lucide-react';
...
export const StepStatusBanner: React.FC = () => {
  ...
  if (!currentResult && !bannerMessage) return null;
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400">
                {currentResult ? `Step ${currentStepIndex + 1} of ${totalSteps}` : 'System Status'}
              </span>
              {currentStep && (
          ...
          <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-lg border border-slate-800">
            ...
            {isAnimating ? (
              ...
              >
                ...
                <span>{isLastStep ? 'Restart' : 'Play'}</span>
              </button>
            )}
            ...
            >
              ...
              <option value={600}>1.8x Fast</option>
            </select>
          </div>
        )}
