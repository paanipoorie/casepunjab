...
import { 
  Eye, Cpu, Layers, CornerDownRight, ArrowRight, 
  MapPin, Info, AlertCircle, RefreshCw 
} from 'lucide-react';
...
export const RouteVisualizer: React.FC = () => {
  ...
  // Helper to resolve conceptual address for next node
  ...
  return (
    <div id="playground-visualizer" className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
        ...
        {/* Scenario Title & Node Count */}
        <div className="flex items-center gap-2.5">
          <div className="h-3 w-3 rounded-full bg-emerald-400 animate-ping"></div>
          <div>
            <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
              <span>{scenario.title} — Active Chain</span>
              <span className="text-xs font-mono px-2 py-0.5 rounded bg-slate-800 text-amber-300 border border-slate-700 font-semibold">
                {nodes.length} {scenario.nodeLabel.toLowerCase()}{nodes.length === 1 ? '' : 's'}
              </span>
            </h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Toggle: Route View vs Pointer View */}
          <div className="flex items-center p-1 rounded-lg bg-slate-950 border border-slate-800 text-xs font-semibold">
            <button
      ...
      {viewMode === 'pointer' ? (
        ...
        /* View Mode 2: Route View (Standard Node Chain) */
        <div 
          ref={containerRef}
          className="bg-slate-900/60 rounded-2xl border border-slate-800/90 p-4 sm:p-6 shadow-inner overflow-x-auto min-h-[260px] flex items-center"
        >
          {nodes.length === 0 ? (
            /* Empty State Fallback */
            <div className="w-full text-center py-12 px-4">
              <AlertCircle className="w-12 h-12 text-slate-500 mx-auto mb-3" />
              <h4 className="text-lg font-bold text-slate-300">The Linked List is Empty</h4>
              <p className="text-sm text-slate-400 max-w-md mx-auto mt-1 mb-6">
                All nodes have been removed (HEAD is currently NULL). You can add a new landmark or reset the tour.
              </p>
              <button
                onClick={executeReset}
                className="px-4 py-2 rounded-lg bg-amber-500 text-slate-950 font-bold text-sm hover:bg-amber-400 inline-flex items-center gap-2"
              >
            ...
            /* Connected Horizontal & Vertical Chain */
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2 w-full lg:w-auto">
              
              {/* HEAD Pointer Tag */}
              <div className="flex lg:flex-col items-center justify-center p-2 rounded-lg bg-amber-500/10 border border-amber-500/30 text-center shrink-0">
                    ...
                    {!isTail ? (
                      ...
                      <div className="shrink-0 flex flex-col items-center justify-center lg:px-2 py-2">
                        ...
                        <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-center font-mono">
                          <span className="text-[10px] text-rose-400 block font-bold">TERMINATOR</span>
                          <span className="text-xs font-bold text-rose-300">NULL (0x0)</span>
                        </div>
                      </div>
                    )}
                  </React.Fragment>
      ...
      {selectedNode && (
        <div className="bg-slate-900 border border-amber-500/40 rounded-xl p-4 animate-fadeIn flex flex-col sm:flex-row items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 mt-1">
                {selectedNode.metadata?.description || `A landmark in Tim's linked itinerary.`}
              </p>
              <div className="mt-2 text-xs font-mono text-slate-400 flex items-center gap-3">
          ...
          <button
            ...
            className="text-xs text-slate-400 hover:text-slate-200 px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700"
          >
            Close Details
          </button>
        </div>
